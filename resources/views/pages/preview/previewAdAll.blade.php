...
<body>
    ...
    @if (!empty($bannerAd) && $bannerAd['type'] == 'banner')
        {{-- <section>
            @foreach ($bannerAd['banner'] as $ad)
            <div class="story-section">
                <img src="{{ $ad['file'] }}" alt="" width="100%">
            </div>
            @endforeach
        </section> --}}
    {{-- @else
    ...
    {{-- @if (!empty($homeBannerAd) && $homeBannerAd['type'] == 'home_banner')
        <section>
            @foreach ($homeBannerAd['home_banner'] as $ad)
            <div class="story-section">
                <img src="{{ $ad['file'] }}" alt="" width="100%">
            </div>
            @endforeach
    ...
    <section>
        <div class="story-section">
            <div class="horizontal-scroll-wrapper">
                @if (!empty($storyAd) && $storyAd['type'] == 'story')
                    @foreach ($storyAd['story'] as $key => $ad)
                    <div class="text-center img-border">
                        <img src="{{ $ad['file'] }}" class="story-item" />
                        <div class="sotry-title">{{ $ad['title'] }}</div>
                    </div>
                    @endforeach
                @endif
    ...
    <section>
        <div class="row">
            ...
            @if (!empty($nativeAd) && $nativeAd['type'] == 'native')
            @php
                $position = max(array_column($nativeAd['native'], 'position'));
            @endphp
            @for ($i = 1; $i < $position + 3; $i++)
                ...
                @foreach ($nativeAd['native'] as $key => $ad)
                @if ($i == $ad['position'])
                <div class="col-md-12">
                    <div class="card custom-card">
                        <div class="card-body p-0">
                            <div class="row pt-3 pl-3 pr-3 mb-2">
                                <div class="pl-3">
                                    <img src="{{ $ad['logo'] }}" alt="" style="border-radius: 50%" width="35px" height="auto">
                                </div>
                                <div class="pl-2">
                            ...
                            <div>
                                <img src="{{ $ad['file'] }}" alt="" width="100%" height="auto">
                            </div>
                            <div class="row m-4">
                                <a href="{{ $ad['link'] }}" type="button" class="btn btn-outline-info font-weight-bold w-100" target="balnk">Open Link</a>
                            </div>
                        </div>
                    </div>
                </div>
