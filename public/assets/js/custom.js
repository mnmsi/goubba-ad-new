...
$('.number-input').keydown(function(event){
    return (event.keyCode === 8 || event.keyCode === 46 || event.keyCode === 13) ? true : !isNaN(Number(event.key));
});

function loadCampData (value, route) {

    if ($('#loadDashboardData').length > 0) {
        $('#loadDashboardData').remove();
    }

    $('#loaderId').show();

    $("#loadViewDiv").load(route + "?campId=" + value, function(response, status, xhr){
        if (status == 'success') {
            $('#loaderId').hide();
        } else{
            $('#loaderId').hide();
...
function loadUserClickTableData (value, route) {

    if ($('#adClickUserTableId').length > 0) {
        $('#adClickUserTableId').remove();
    }

    $('#loaderId').show();

    $("#adClickUserTable").load(route + "?campId=" + value, function(response, status, xhr){
        if (status == 'success') {
            $('#loaderId').hide();
        } else{
            $('#loaderId').hide();
...
function fnUserDataLoad (value, route) {
    ...
    if (value == 1) {
        $('#adminStatsDiv').show('slow');
    } else {
        $('#adminStatsDiv').hide('slow');
    }

    $('#loaderId').show();

    $("#loadViewDiv").load(route + "?userId=" + value, function(response, status, xhr){
        if (status == 'success') {
            $('#loaderId').hide();
        } else{
            $('#loaderId').hide();
